...
%%读取数据
data_acc = load('../DATA/Mobel_Phone/acc.txt');
data_gyro = load('../DATA/Mobel_Phone/gyro.txt');
data_mag = load('../DATA/Mobel_Phone/mag.txt');
datasize = length(data_acc);

timestamp_acc = data_acc(:,1);
timestamp_gyro = data_gyro(:,1);
timestamp_mag = data_mag(:,1);
...
mag_s = interp1(timestamp_mag, mag_s, timestamp_acc, 'spline' ,'extrap')';
...
% 卡尔曼矩阵参数
delta_x=[0; 0; 0;
    1.787629e-02;    -5.342901e-03;    -5.551020e-02;
    0; 0; 0;
    0; 0; 0;
    -8.546124e-02;    -4.346455e-02;    9.786575e+00-g];
...
%% 主循环
for t = 2:datasize
    dt = ((timestamp_acc(t) - timestamp_acc(t-1)))*(1e-9); %将纳秒换算成秒
    ...
    %通过磁力计来计算yaw角
    yaw_compass = get_the_north_angle(mag_s(:,t), acc_s(:,t));
    compass_array(t) = yaw_compass;
    delta_compass = yaw_compass -  compass_array(t-1);
    
    if(abs(delta_compass) < yaw_threshold)
        delta_yaw_compass = delta_compass;
    else
        delta_yaw_compass = 0;
    end
