...
% data = data(1:720000, 3:5)*3600;  %截取保温两个小时后的，五个小时的数据，把 deg/s 转为 deg/h  
...
xlabel('平均时间:sec');                 %添加x轴标签  
ylabel('Allan标准差:deg/h');              %添加y轴标签  
...
Q = sqrt(abs(C(:, 1) / 3));             %量化噪声，单位：deg/h  ，  曲率-1
N = sqrt(abs(C(:, 2) / 1)) / 60;    %角度随机游走，单位：deg/h^0.5  曲率-1/2
Bs = sqrt(abs(C(:, 3))) / 0.6643;   %零偏不稳定性，单位：deg/h      曲率0
